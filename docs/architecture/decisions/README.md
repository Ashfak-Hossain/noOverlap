...
The numbering has gaps because a few records are reserved for decisions that belong to parts of the
system still being built, such as the outbox relay mechanism and trace propagation across the queue.
They are added when that work lands, rather than written speculatively ahead of it.
